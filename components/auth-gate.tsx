...
export type AuthUser = { id: string; email: string; name: string; role: "USER" | "ADMIN"; status: string; refreshUser: () => Promise<void> };
...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  const [user, setUser] = useState<Omit<AuthUser, "refreshUser"> | null>(null);
  const refreshUser = useCallback(async () => {
    const value = await apiFetch<Omit<AuthUser, "refreshUser">>("/me");
    setUser(value);
    setState("signed-in");
  }, []);
  useEffect(() => {
    apiFetch<Omit<AuthUser, "refreshUser">>("/me")
      .then((value) => { setUser(value); setState("signed-in"); })
  ...
  if (state === "signed-in" && user) return <AuthContext.Provider value={{ ...user, refreshUser }}>{children}</AuthContext.Provider>;

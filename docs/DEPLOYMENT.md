...
当前仓库提供了针对已知旧版自动建表结构的一次性接管命令。仅当诊断结果
显示全部应用表存在、迁移历史为空，并且已完成数据库备份时执行：
...
接管命令会补齐 `orders.months`（按订单周期回填 1/3/12）、
插件配置、权限、Cookie 检查和支付快照字段，然后记录当前全部迁移哈希。
它不会删除表或业务行；MySQL 的 `ALTER TABLE` 可能独立提交，失败后可在
同一备份上复查并安全重跑。若缺少应用表、订单周期异常或迁移历史不为空，
命令会停止，必须人工制定数据迁移方案。

当前版本的 `npm run db:migrate` 也会在检测到“应用表存在但 Drizzle 历史为空”
时自动调用同一接管逻辑；`npm run db:adopt` 仅用于需要单独观察接管输出的场景。

先备份数据库，应用迁移，再启动 API、Worker 和 Web。生产命令
`npm run start:stack` 会先执行幂等迁移；API 和 Worker 启动前也会通过
